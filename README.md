...
## v1.5.1 更新记录（2026-08-07）
- 响度校验改为播放优先：开始/停止一键控制，已成功校验的歌曲不会重复校验；播放开始时，后台校验立即暂停并等待播放空闲后恢复，失败仅以轻提示告知。
- 新增“校验失败”筛选。失败歌曲可由用户选择输出文件夹，转换为 FLAC 并重新校验；只有转换和校验都成功后才会出现删除原文件的二次确认，选择保留或任一步失败都会保留原文件。
- 长按歌曲进入多选，可批量移到应用内个人文件夹或从应用音乐库移除；后者绝不删除手机原始音频。单选时可编辑歌曲名称、歌手名称，仅影响本应用的显示、搜索与分类。
- 播放栏显示上一首和下一首的实际播放顺序；锁屏后继续使用 Android 系统媒体卡片切歌。
- 智能分类可选择将同一已知歌手归入同一组；增加回到列表顶部按钮。
- 版本号：`1.5.1`（`versionCode 8`），延续固定发布签名，覆盖安装会保留应用内歌曲、文件夹、歌词和设置。

## v1.5.0 更新记录（2026-08-07）

- 新增 M4A、AAC、OGG、Opus、WMA 的导入、播放与响度校验；系统解码不可用时自动回退到内置 FFmpeg，不保存转换文件。
- 新增棕色主题，并在原有侧边栏主题选择器中显示。
- 新增 Android 系统媒体会话与前台播放服务：熄屏或锁屏后可从系统媒体卡片进行上一首、播放/暂停、下一首、循环及随机播放控制。
- 个人文件夹被选中后，顺序播放、单曲循环与随机播放均只在该文件夹已有歌曲中进行。
- 响度校验队列提高为最多两首并行；保持 APE/WMA 的内存/管道实时解码，不产生音频副本。
- 版本号：`1.5.0`（`versionCode 7`），沿用 v1.4.0 起的固定发布签名，覆盖更新会保留应用内歌曲、文件夹、歌词和设置。
